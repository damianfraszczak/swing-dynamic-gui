...
public class SwingUtils {
    ...
    public static JMenuItem createMenuItemFromAction(Action action) {
        ...
        } else if (action.getValue(Action.LONG_DESCRIPTION) != null) {
            text = action.getValue(Action.LONG_DESCRIPTION).toString();
        } else if (action.getValue(Action.SHORT_DESCRIPTION) != null) {
            text = action.getValue(Action.SHORT_DESCRIPTION).toString();
        }
    ...
    public static JPopupMenu createPopUpMenuFromActionsAndMenuItems(
            Collection items) {
        JPopupMenu menu = new JPopupMenu();
        if (items != null)
            items.forEach(action -> menu
                    .add(action instanceof Action ? createMenuItemFromAction((Action) action)
                            : (Component) action));
        return menu;
    }
}
